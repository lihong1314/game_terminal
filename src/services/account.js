 ...
 * @return {*|Promise<any>}
 */
const getRemoteLogin = code => post(
  'https://api.sit.ffan.com/microapp/v1/wxLogin',
  { jsCode: code, appId: 'wx99841f8281a779b0' }
)
 ...
 * @return {*|Promise<any>}
 */
exports.bindAccountBySilent = ({ encryptedData, iv, ...params } = {}) => getSession()
  .then(res => {
    if (!encryptedData || !iv) {
      return reject(`-- The params of wx.getUserInfo() is invalid,
      \n -- which reference: 
      \n https://mp.weixin.qq.com/debug/wxadoc/dev/api/open.html`)
    } else {
      console.log('encryptedData,iv,res',encryptedData,iv,res)
 ...
 * @return {*|Promise<any>}
 */
exports.bindAccountByCode = ({ mobile = '', verifyCode = '', ...params } = 0) => getSession()
  .then(res => {
    if (!checkMobile(mobile) || !verifyCode.trim()) {
      return reject('-- The parameter of mobile, verifyCode must be required')
    } else {
      return post(
        'https://api.sit.ffan.com/microapp/v1/ffanLogin', {
          ...{
            mobile,
            verifyCode,
            plazaId: 0,
 ...
 * @return {*|Promise<any>}
 */
exports.getUserInfo = (isForceRefresh = 0) => getSession()
  .then(res => post('https://api.sit.ffan.com/microapp/v1/userInfo', {
    wxFfanToken: res,
...
exports.sendVerifyCode = mobile => {
  if (!checkMobile(mobile)) {
    ...
    return post('https://api.sit.ffan.com/wechatxmt/v1/member/verifyCode?sign=xcx',
      {mobile,type:2 }
    )
  }
}

// http://api.sit.ffan.com/wechatxmt/v1/member/verifyCode?mobile=13601310087&type=2

/**
 * 注销帐号
 * @type {*|Promise<any>|Promise.<TResult>}
 */
exports.logout = () => getSession().then(session => new Promise((resolve, reject) => {
